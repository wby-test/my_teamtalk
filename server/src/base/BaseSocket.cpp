...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	m_socket = socket(AF_INET, SOCK_STREAM, 0);  ////only suport ipv4
...
{
	int ret = setsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, &send_size, sizeof(uint32_t));
	if (ret == SOCKET_ERROR) {
...
{
	int nodelay = 1;
	////禁用nagle算法，使得内核缓冲区不用等到足够的数据就可以发送
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	char ip_str[64]; //ipv6 128位，v4 32位
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		////ntohl/ntohs 网络字节序转主机字节序， windows不需要调用WSAstartup函数即可以使用。
